## PA0
df -h :to see how much disk space Ubuntu occupies.
poweroff:n
...
3.To exit the Vim type:\<ESC>  :q! \<ENTER> to trash all the changes
or type:\<ESC>  :wq!\<ENTER> to save the changes.
4.To delete the character at the cursor type:x
5.To insert or append text type:

    i   type inserted text  \<ESC>    
    A   type append text    \<ESC>
...
where:
  operator is what to do,such as d for delte
  [number] is an optional count to repeat the motion
  motion moves over the text to operte on,such as w(word),e(end of word),$(end of the line),etc.
...
After a search type n to find the next occurrence in the same derection or N to search in the opposite direction.
...
5.:r !dir reads the output of the dir command and puts it below the ursor position.
...
#### Addition
1.NOTE
To move up page by page type CTRL U
To move down page by page type CTRL D
...
T{char}:before [count]'th occurrence of {char} to the left.The cursor is placed on the character left of {char} |inclusive|.

3.NOTE
To repeat the operation before,just type "." .